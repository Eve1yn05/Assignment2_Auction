...
public class ModernArt {
    ...
    public void dealPainting(int round) {
        ...
        for (int i = 0; i<noOfPlayers;i++){
            for (int j=0;j<paintingsToDeal;j++){
                if (!deck.isEmpty()){
                    Painting painting = deck.remove(deck.size()-1);
                    players[i].dealPaintings(painting);
    ...
    public int[] updateScoreboard(int round, int[] paintingCount) {
       int [] currentScore = new int[Painting.ARTIST_NAMES.length];  // store score
       boolean [] receivedScore = new boolean[Painting.ARTIST_NAMES.length];
       for (int i=0; i<Painting.ARTIST_NAMES.length;i++){
           scoreboard[round][i]=0;  // reset their score = 0
       }
       for (int rank=0; rank<3;rank++){
           int maxIndex =-1;
           for (int i=0; i<paintingCount.length;i++) {
               if (!receivedScore[i]) {   // this artist still not receive a score
                   if (maxIndex == -1 || paintingCount[i] > paintingCount[maxIndex]) {
                       maxIndex = i;
                   }
               }
           }
           if (maxIndex != -1){
               if (rank ==0){
                   scoreboard[round][maxIndex]=30;
               } else if (rank==1) {
                   scoreboard[round][maxIndex]=20;

               } else if (rank==2) {
                   scoreboard[round][maxIndex]=10;

               }
               receivedScore[maxIndex]=true;    // this artist is received

           }
       }
       for (int i =0; i<scoreboard[round].length;i++){
           if (scoreboard[round][i] != 0){
               currentScore[i]= scoreboard[round][i];
               for (int j=0 ; j<round;j++){
                   currentScore[i] += scoreboard[j][i];  // sum previous round
               }
           }
    ...
    public void startgame() {
        ...
        for (int round = 0; round < ROUND; round++) {
            //deal the paintings
            dealPainting(round);
            // dealing paintings during each round start
            // according to the PRE_DEAL
